...
def main():
    ...
    global_q_net1 = QNet(INPUT_DIM, EMBEDDING_DIM, ACTION_FEATURE_DIM).to(device)
    global_q_net2 = QNet(INPUT_DIM, EMBEDDING_DIM, ACTION_FEATURE_DIM).to(device)
    log_alpha = torch.FloatTensor([-2]).to(device)  # not trainable when loaded from checkpoint, manually tune it for now
    log_alpha.requires_grad = True

    global_target_q_net1 = QNet(INPUT_DIM, EMBEDDING_DIM, ACTION_FEATURE_DIM).to(device)
    global_target_q_net2 = QNet(INPUT_DIM, EMBEDDING_DIM, ACTION_FEATURE_DIM).to(device)
        ...
        while job_list:
            ...
            if curr_episode % 1 == 0 and len(experience_buffer[0]) >= MINIMUM_BUFFER_SIZE:
                ...
                for j in range(8):
                    ...
                    action_inputs_batch = torch.stack(rollouts[2]).to(device)
                    current_inputs_batch = torch.stack(rollouts[3]).to(device)
                    node_padding_mask_batch = torch.stack(rollouts[4]).to(device)
                    edge_padding_mask_batch = torch.stack(rollouts[5]).to(device)
                    edge_mask_batch = torch.stack(rollouts[6]).to(device)
                    ...
                    next_action_inputs_batch = torch.stack(rollouts[12]).to(device)
                    next_current_inputs_batch = torch.stack(rollouts[13]).to(device)
                    next_node_padding_mask_batch = torch.stack(rollouts[14]).to(device)
                    next_edge_padding_mask_batch = torch.stack(rollouts[15]).to(device)
                    next_edge_mask_batch = torch.stack(rollouts[16]).to(device)

                    # SAC
                    with torch.no_grad():
                        q_values1, _ = dp_q_net1(node_inputs_batch, edge_inputs_batch, action_inputs_batch, current_inputs_batch, node_padding_mask_batch, edge_padding_mask_batch, edge_mask_batch)
                        q_values2, _ = dp_q_net2(node_inputs_batch, edge_inputs_batch, action_inputs_batch, current_inputs_batch, node_padding_mask_batch, edge_padding_mask_batch, edge_mask_batch)
                        q_values = torch.min(q_values1, q_values2)

                    logp = dp_policy(node_inputs_batch, edge_inputs_batch, action_inputs_batch, current_inputs_batch, node_padding_mask_batch, edge_padding_mask_batch, edge_mask_batch)
                    ...
                    with torch.no_grad():
                        next_logp = dp_policy(next_node_inputs_batch, next_edge_inputs_batch, next_action_inputs_batch, next_current_inputs_batch, next_node_padding_mask_batch, next_edge_padding_mask_batch, next_edge_mask_batch)
                        next_q_values1, _ = dp_target_q_net1(next_node_inputs_batch, next_edge_inputs_batch, next_action_inputs_batch, next_current_inputs_batch, next_node_padding_mask_batch, next_edge_padding_mask_batch, next_edge_mask_batch)
                        next_q_values2, _ = dp_target_q_net2(next_node_inputs_batch, next_edge_inputs_batch, next_action_inputs_batch, next_current_inputs_batch, next_node_padding_mask_batch, next_edge_padding_mask_batch, next_edge_mask_batch)
